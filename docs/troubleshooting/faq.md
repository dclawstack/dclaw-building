...
### How do I update DClaw Building?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp building --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Building without Kubernetes?

Yes, for local development:

```bash
cd dclaw-building/frontend && npm run dev
cd dclaw-building/backend && uvicorn dclaw_building.main:app --reload
```
