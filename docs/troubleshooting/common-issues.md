...
# Check events
kubectl get events -n dclaw-building --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-building deployment/dclaw-building-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-building-db-credentials -n dclaw-building
   ```
